...
def start_crawl(url):
    ...
    driver.get(url)
    info = driver.find_element_by_id("divPlayerList").find_element_by_css_selector("div.tr")
    player = info.find_element_by_css_selector("div.players_utils")
    player_detail = player.find_element_by_css_selector("a.btn_detail_link")
    actions = webdriver.ActionChains(driver).move_to_element(player).click(player_detail)
    actions.perform()
    driver.implicitly_wait(3)
    player_name = driver.find_elements_by_class_name("name")
    content = driver.find_element_by_class_name("content_middle")
    stats = content.find_elements_by_class_name("ab")
